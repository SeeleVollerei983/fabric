...
func CreateRealEstate(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if accountId == proprietor {
		return shim.Error("操作人应为病人且与所有人不能相同")
	}
	// 参数数据格式转换
	...
	if err != nil || len(resultsProprietor) != 1 {
		return shim.Error(fmt.Sprintf("医生proprietor信息验证失败%s", err))
	}
...
// QueryRealEstateList 查询房地产(可查询所有，也可根据所有人查询名下病历)
