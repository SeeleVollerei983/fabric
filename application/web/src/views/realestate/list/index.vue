<template>
  <div class="container">
    ...
    >
      ...
      <p>用户名: {{ userName }}</p>
      <p>余额: ￥{{ balance }} 元</p>
      <p>当发起出售、转移病历或质押操作后，转移状态为true</p>
      <p>当转移状态为false时，才可发起出售、转移病历或质押操作</p>
    </el-alert>
    ...
    <el-row v-loading="loading" :gutter="20">
      <el-col v-for="(val,index) in realEstateList" :key="index" :span="6" :offset="1">
        <el-card class="realEstate-card">
          ...
          <div class="item">
            <el-tag type="warning">患者信息: </el-tag>
            <span>{{ val.totalArea }} </span>
          </div>
          <div class="item">
            <el-tag type="danger">就诊记录: </el-tag>
          ...
          <div v-if="!val.encumbrance&&roles[0] !== 'admin'">
            ...
            <el-button type="text" @click="openDonatingDialog(val)">转移病历</el-button>
          </div>
    ...
    <el-dialog v-loading="loadingDialog" :visible.sync="dialogCreateDonating" :close-on-click-modal="false" @close="resetForm('DonatingForm')">
      <el-form ref="DonatingForm" :model="DonatingForm" :rules="rulesDonating" label-width="100px">
        <el-form-item label="医生" prop="proprietor">
          <el-select v-model="DonatingForm.proprietor" placeholder="请选择医生" @change="selectGet">
            <el-option
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="createDonating('DonatingForm')">立即转移病历</el-button>
      ...
      queryAccountList().then(response => {
        if (response !== null) {
          ...
          this.accountList = response.filter(item =>
            item.userName !== '病人' && item.accountId !== this.accountId
          )
        }
      })
    },
