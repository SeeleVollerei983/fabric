...
type DonatingRequestBody struct {
	ObjectOfDonating string `json:"objectOfDonating"` //转移病历对象
	Donor            string `json:"donor"`            //转移病历人
...
func CreateDonating(c *gin.Context) {
	...
	if body.ObjectOfDonating == "" || body.Donor == "" || body.Grantee == "" {
		appG.Response(http.StatusBadRequest, "失败", "ObjectOfDonating转移病历对象和Donor转移病历人和Grantee受赠人不能为空")
		return
	}
	var bodyBytes [][]byte
